...
const HEADER = `// ====== Generated from librdkafka ${LIBRDKAFKA_VERSION} source ======`;
...
async function execCommand(command, options) {
  return new Promise((res, rej) => {
    ...
    cmd.on('close', (code) => {
      if (code != 0) {
        rej(new Error(`Command returned unsuccessful code: ${code}`));
        return;
      }
      res(output.trim());
    });
...
function generateConfigDTS(file) {
  ...
  let output = `${HEADER}
// Code used to generate this is a derivative work of the code from the Nam Nguyen
...
function updateErrorDefinitions(file) {
  ...
  const body = m[1]
    ...
    .replace(`  /* Internal errors to rdkafka: */\n`, '')

  const error_js_file = path.resolve(__dirname, '../lib/error.js');
  const error_js = fs.readFileSync(error_js_file)
    .toString()
    .replace(/LibrdKafkaError.codes = {[^}]+/g, `${HEADER}\nLibrdKafkaError.codes = {\n${body}`)

  fs.writeFileSync(error_js_file, error_js);
  fs.writeFileSync(path.resolve(__dirname, '../errors.d.ts'), `${HEADER}\nexport const CODES: { ERRORS: {${body.replace(/-?\d+,/g, 'number')}}}`)
}

(async function updateTypeDefs() {
  // await execCommand('git fetch --tags', { cwd: LIBRDKAFKA_DIR });
  // await execCommand(`git checkout tags/v${LIBRDKAFKA_VERSION}`, { cwd: LIBRDKAFKA_DIR });
  // generateConfigDTS(fs.readFileSync(path.resolve(LIBRDKAFKA_DIR, 'CONFIGURATION.md')).toString());
